...
SELECT produto, quantidade_vendida, valor_total
FROM(
    SELECT p.nome AS produto, SUM(ip.quantidade) AS quantidade_vendida, SUM(ip.quantidade * ip.preco_unitario) AS valor_total
...
SELECT ROUND(AVG(total_pedidos), 2) AS media_pedidos_por_cliente, ROUND(AVG(valor_total), 2) AS media_valor_por_cliente
FROM(
