...
-- =================================================================
-- DESAFIOS FINAIS DO MODULO 03
-- =================================================================

-- Desafio Final 1: Relatorio de Contato Padronizado
-- Crie um relatorio para a equipe de marketing que contenha:
-- - O nome completo do cliente em maiusculas.
-- - O email do cliente em minusculas.
-- - Uma coluna "Localizacao" formatada como "Cidade (UF)". Ex: "Sao Paulo (SP)".
SELECT
    UPPER(nome) AS nome_maiusculo,
...
-- Desafio Final 2: Analise de Marcas
-- Encontre todas as marcas unicas de produtos.
-- Exiba o nome da marca em maiusculas e o numero de caracteres do nome da marca.
-- Ordene pelo nome da marca.
SELECT DISTINCT
    UPPER(marca) AS marca_maiuscula,
    LENGTH(marca) AS tamanho_marca
FROM produtos
WHERE marca IS NOT NULL
ORDER BY marca_maiuscula;


-- Desafio Final 3: Nomes Curtos de Produtos
-- Liste todos os produtos cujos nomes (sem espacos no inicio ou fim) tenham 10 caracteres ou menos.
-- Exiba o nome e o comprimento do nome.
SELECT
    TRIM(nome) AS nome_limpo,
    LENGTH(TRIM(nome)) AS tamanho
FROM produtos
WHERE LENGTH(TRIM(nome)) <= 10;


-- Desafio Final 4: (Avancado) Gerador de Nome de Usuario
-- Crie uma sugestao de nome de usuario para cada cliente.
-- O nome de usuario deve ser:
-- - As 3 primeiras letras do nome (em minusculas).
-- - Concatenado com o comprimento total do nome completo.
-- - Exemplo: Para "Ana Clara", o resultado seria "ana9".
SELECT
    nome,
    CONCAT(
        LOWER(SUBSTRING(nome FROM 1 FOR 3)),
        LENGTH(nome)
    ) AS sugestao_usuario
FROM clientes;
